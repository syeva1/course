...
of strings. Once we've got that data, we are all done.
...
say to Claude, thanks for the JSON, we're done and we're
...
we can provide a tool_choice parameter when

we call the client.messages.create function. Specifically,

we can provide a tool_choice that is a dictionary with
...
a name of analyze_financial_statement, we would

put in analyze_financial_statement right there like so.
...
So in the cell right above, helper_functions, I'll
...
we want to make sure that our chat function can take in a tool_choice

argument, and that the tool_choice will be passed

on through to our actual client.messages

.create function call. So back over here,

I'm going to add in yet another argument of tool_choice.

And again, I'll default it to be none. We're

then going to set it up in the same way that we set up tools right here. So

if a tool_choice is provided, then
...
the article_summary_schema.

And then finally, we want to force

Claude to call that particular tool. So we'll put

in a tool_choice, which

will be a dictionary with a type of

tool and a name of article_summary.
...
response message, we have a tool_use block that

has an inputs as usual. And the inputs here

is going to be a dictionary with all the data that we asked for.

So there is a title, there is an author, and

then key_insights, which is going to be a list of
...
.message[0].tool_calls[0].function.arguments
